...
float perlin_noise(vec2 point) {
	...
	*/
	...
	vec2 floor_corn = floor(point);

	vec4 sutv = vec4(0.);
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 2; j++) {
			vec2 corn = floor_corn + vec2(i, j);
			vec2 grad = gradients(hash_func(corn));

			sutv[2*i + j] = dot(grad, point - corn);
		}
	}
...
float turbulence(vec2 point) {
	...
	*/
	...
	for(int i = 0; i < num_octaves; i++) {
		turb += A * abs(perlin_noise(point * w));
		w *= freq_multiplier;
		A *= ampl_multiplier;
	}
	return turb;
...
vec3 tex_map(vec2 point) {
	...
	*/
	// below water: render water
	float s = perlin_fbm(point);
	if (s < terrain_water_level) {
		return terrain_color_water;
	}
	// above water: render grass nd mountains
	float alpha = s - terrain_water_level;
	vec3 res = mix(terrain_color_grass, terrain_color_mountain, alpha);
	return res;
}
...
vec3 tex_wood(vec2 point) {
	...
	*/
	...
	vec3 res = mix(brown_dark, brown_light, alpha);
	return res;
}
...
vec3 tex_marble(vec2 point) {
	...
	*/
	float s1 = perlin_fbm(point);
	float s2 = perlin_fbm(point + vec2(1.7, 4.6));
	vec2 q = vec2(s1, s2);

	float s = perlin_fbm(point + 4. * q);
	float alpha = .5 * (1. + s);
	vec3 res = mix(white, brown_dark, alpha);
	return res;
}
