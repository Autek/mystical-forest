...
GL2.3: To solve this task, we first computed the position and normal vectors using the model-view and normals-to-view matrices. Then we computed the light and view directions, with the view direction pointing towards the camera. Then we obtained the ambient, diffuse and specular components as usual and used them to get the color. That color is passed to the fragment shader via the `varying` keyword.

GL2.4: To solve this task, just like the previous task we start off by computing the position and normal vectors with the model-view and normals-to-views matrices as well as the light and view directions, with the view direction pointing towards the camera. All of this done in the vertex shader, and we pass the normal, light and view directions to the fragment shader via the `varying` keyword. Then unlike the previous task, we calculate the color in the fragment shader by calculating the ambient, diffuse and specular components as indicated in the Phong model.
